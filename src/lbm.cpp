...
void LBM_Domain::allocate(Device& device) {
	...
	if (Settings::IsFeatureEnabled(Feature::MOVING_BOUNDARIES))
	...
	if (Settings::IsFeatureEnabled(Feature::PARTICLES))
	{
...
string LBM_Domain::device_defines() const {
	...
	if (Settings::IsFeatureEnabled(Feature::EQUILIBRIUM_BOUNDARIES))
		ss << "\n #define EQUILIBRIUM_BOUNDARIES";
	if (Settings::IsFeatureEnabled(Feature::FORCE_FIELD))
		ss << "\n #define FORCE_FIELD";
	if (Settings::IsFeatureEnabled(Feature::SUBGRID))
		ss << "\n #define SUBGRID";
...
string LBM_Domain::Graphics::device_defines() const {
	...
	if (GraphicsSettings::IsAlphaEnabled())
		ss << "\n #define GRAPHICS_TRANSPARENCY " << to_string(GraphicsSettings::GetAlpha()) << "f";
...
void LBM::sanity_checks_constructor(const vector<Device_Info>& device_infos, const uint Nx, const uint Ny, const uint Nz, const uint Dx, const uint Dy, const uint Dz, const float nu, const float fx, const float fy, const float fz, const float sigma, const float alpha, const float beta, const uint particles_N, const float particles_rho) { // sanity checks on grid resolution and extension support
	...
	else
	{
		...
		{
			if(fx==0.0f&&fy==0.0f&&fz==0.0f) 
				print_warning("The VOLUME_FORCE extension is enabled but the volume force in LBM constructor is set to zero.");
		}
	}
	if (!Settings::IsFeatureEnabled(Feature::SURFACE))
	{
		if(sigma!=0.0f) 
			print_error("Surface tension is set in LBM constructor in main_setup(), but SURFACE is not enabled.");
	}
	if (Settings::IsFeatureEnabled(Feature::TEMPERATURE))
	{
		if(alpha==0.0f&&beta==0.0f) 
			print_warning("The TEMPERATURE extension is enabled but the thermal diffusion/expansion coefficients alpha/beta in the LBM constructor are both set to zero.");
	}
	else
	{
		if(alpha!=0.0f||beta!=0.0f) 
			print_error("Thermal diffusion/expansion coefficients are set in LBM constructor in main_setup(), but TEMPERATURE is not enabled.");
	}
	if (Settings::IsFeatureEnabled(Feature::PARTICLES))
	{
		if(particles_N==0u) 
		...
		if (!Settings::IsFeatureEnabled(Feature::VOLUME_FORCE) || !Settings::IsFeatureEnabled(Feature::FORCE_FIELD))
		{
			if(particles_rho!=1.0f) 
				print_error("Particle density is set unequal to 1, but particle-fluid 2-way-coupling is not enabled. Enable both VOLUME_FORCE and FORCE_FIELD.");
		}
		if (Settings::IsFeatureEnabled(Feature::FORCE_FIELD))
		{
			if(particles_rho==1.0f) 
				print_warning("Particle density is set to 1, so particles behave as passive tracers without acting a force on the fluid, but particle-fluid 2-way-coupling is enabled.");
		}
	}
	else
	{
...
void LBM::sanity_checks_initialization() { // sanity checks during initialization on used extensions based on used flags
	...
	if (Settings::IsFeatureEnabled(Feature::MOVING_BOUNDARIES))
	{
		if(!moving_boundaries_used) 
			print_warning("The MOVING_BOUNDARIES extension is enabled but no moving boundary nodes (TYPE_S flag and velocity unequal to zero) are placed in the simulation box.");
	}
	else
	{
		if(moving_boundaries_used) 
			print_warning("Some boundary nodes have non-zero velocity, but MOVING_BOUNDARIES is not enabled.");
	}
	if (Settings::IsFeatureEnabled(Feature::EQUILIBRIUM_BOUNDARIES))
	{
		if(!equilibrium_boundaries_used) 
			print_warning("The EQUILIBRIUM_BOUNDARIES extension is enabled but no equilibrium boundary nodes (TYPE_E flag) are placed in the simulation box.");
	}
	else
	{
		if(equilibrium_boundaries_used) 
			print_error("Some nodes are set as equilibrium boundaries with the TYPE_E flag, but EQUILIBRIUM_BOUNDARIES is not enabled.");
	}
	if (Settings::IsFeatureEnabled(Feature::SURFACE))
	{
		if(!surface_used) 
			print_error("The SURFACE extension is enabled but no fluid/interface/gas nodes (TYPE_F/TYPE_I/TYPE_G flags) are placed in the simulation box.");
	}
	else
	{
		if(surface_used) 
			print_error("Some nodes are set as fluid/interface/gas with the TYPE_F/TYPE_I/TYPE_G flags, but SURFACE is not enabled.");
	}
	if (!Settings::IsFeatureEnabled(Feature::TEMPERATURE))
	{
		if(temperature_used) 
			print_error("Some nodes are set as temperature boundary with the TYPE_T flag, but TEMPERATURE is not enabled.");
	}
}
